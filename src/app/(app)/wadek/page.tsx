  ...
  return (
    <div className="silapa-fade" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ padding: '16px 24px', borderBottom: `1px solid ${colors.rowBorder}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 13.5, fontWeight: 700, color: colors.ink }}>Sorotan: mahasiswa belum mengisi KRS</span>
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: colors.subtle }}>
              {['NPM', 'Nama', 'Prodi', 'Dosen PA'].map((h) => <th key={h} style={TH}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {agg.krsFlag.length === 0 && (
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      <thead>
        <tr style={{ background: colors.subtle }}>
          ...
          <th style={{ ...TH, textAlign: 'right' }}>IPK Rata-rata</th>
...
function DrilldownListByProdi({ records, dosenNamaByUid, drilldownKey }: { records: MahasiswaRecord[]; dosenNamaByUid: Map<string, string>; drilldownKey: DrilldownKey }) {
  ...
  const grouped = groupByProdi(rows);
